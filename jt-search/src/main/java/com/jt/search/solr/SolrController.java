...
import com.jt.search.vo.SolrParam;
import com.jt.search.vo.SolrResult;
...
public class SolrController {
	...
	public List<SolrResult> search(String q) {
		try {
			...
			// 分页
			//params.setStart(param.getStart());
			params.setStart(0);
			params.setRows(20);
			...
			// 只查询指定域
			params.set("fl", "id,product_title,product_price,product_image");
...
//			params.setHighlightSimplePost("</span>");
			...
			for (SolrDocument result : results) {
				...
				solrResult.setId(result.get("id").toString());
				solrResult.setTitle(result.get("product_title").toString());
				//solrResult.setNum(result.get("product_num").toString());
				solrResult.setPrice(result.get("product_price").toString());
				solrResult.setImg(result.get("product_image").toString());
				list.add(solrResult);
			}
			// return highlight;
			return list;
